...
Generates final structured intelligence reports in JSON, Markdown, and HTML formats.
"""
...
class Block5ReportBuilder:
    ...
    Трансформує 5-шарову матрицю висновків та метрики істинності в документи.
    """

    def __init__(self):
        # Дефолтний шлях, який перевизначається через main.py
    ...
    def build(self, analytics_result: AnalyticsResult) -> GeneratedReport:
        """
        Головний метод збірки звіту. Зводить воєдино тексти та математичні індекси.
        """
        logger.info("=" * 60)
        logger.info("[Reporter] 📄 БЛОК 5 — REPORT BUILDER | Пакування результатів...")
        logger.info("=" * 60)

        raw_id = analytics_result.query_id
        report_id = f"REP_{raw_id.split('_')[1] if '_' in raw_id else 'DATA'}"
        report = GeneratedReport(report_id, raw_id)

        # Зчитуємо динамічні метрики з нашого оновленого Аналітичного ядра
        metrics = analytics_result.layers_output
        summary = analytics_result.to_dict().get("summary_metrics", {})
        
        total_sig = summary.get("total_signals_verified", 0)
        direct_facts = summary.get("direct_facts_detected", 0)
        mirrors = summary.get("manipulations_inverted", 0)
        avg_mass = summary.get("average_signal_mass", 0.0)
        integrity = summary.get("intelligence_integrity_index", "SECURE")

        report.raw_data = analytics_result.to_dict()
        report.executive_summary = (
            f"Аналітичне дослідження за запитом ID {analytics_result.query_id}. "
            f"Інтегральний коефіцієнт впевненості системи: "
            f"{round(analytics_result.overall_confidence * 100, 1)}%. "
            f"Оброблено джерел: {total_sig} (Прямих фактів: {direct_facts}, "
            f"Дешифровано маніпуляцій: {mirrors}). "
            f"Середня аналітична маса сигналу: {avg_mass}."
        )
...
## 🎯 2. КОНТУР ОЦІНКИ ІСТИННОСТІ (Паспорт 5 Факторів)
- Перевірено первинних сигналів: **{total_sig}**
- Верифіковано прямих фактів: **{direct_facts}**
- Інвертовано дзеркальних маніпуляцій: **{mirrors}**
- Стан інформаційного поля: **`{integrity}`**

## 🔍 3. ВИЯВЛЕНІ ВУЗЬКІ МІСЦЯ ІНФРАСТРУКТУРИ (Е. Ґолдратт / КНР)
{bt_list if analytics_result.bottlenecks else "- Критичних обмежень ємності не зафіксовано."}

## 🔮 4. ЙМОВІРНІСНИЙ ПРОГНОЗ РЕЖИМУ (А. Марков)
{analytics_result.forecast_summary}

## 💡 5. АДВІЗОРІ-НОТАТКИ ДЛЯ ВАЙБ-КОДЕРА
{analytics_result.advisory_notes}

---
*Generated autonomously by OSINT-REILLY Engine Core v4.2*
"""

        # 3. ГЕНЕРАЦІЯ ШАРУ HTML (Для Streamlit Web UI)
        logger.info("[Reporter] Збірка веб-шару HTML з CSS-стилізацією...")
        bt_html = "".join([
            f"<li style='color: #ff4b4b; font-weight: bold;'>{b}</li>" 
            for b in analytics_result.bottlenecks
        ])
        
        report.html_body = f"""
        <div style="background-color: #1e1e1e; color: #ffffff; padding: 25px; border-radius: 10px; font-family: 'Segoe UI', sans-serif;">
            ...
            <table style="width:100%; margin-top:15px; border-collapse: collapse; background:#252525; border-radius:5px;">
                <tr style="border-bottom:1px solid #333;">
                    <td style="padding:10px;">📊 Всього сигналів: <strong>{total_sig}</strong></td>
                    <td style="padding:10px; color:#4cd137;">🎯 Прямі факти: <strong>{direct_facts}</strong></td>
                    <td style="padding:10px; color:#00a8ff;">🔄 Дзеркала: <strong>{mirrors}</strong></td>
                </tr>
            </table>
            
            <div style="margin-top: 20px; background-color: #2d2d2d; padding: 15px; border-left: 5px solid #ff4b4b; border-radius: 4px;">
                <h4 style="margin: 0 0 10px 0; color: #ff4b4b;">⚠️ Критичні обмеження (Теорія Обмежень / КНР):</h4>
            ...
            <div style="text-align: right; margin-top: 20px; font-size: 0.8em; color: #666;">
                Core ID: {report.report_id} | Integrity: {integrity} | Engine v4.2
            </div>
        </div>
        """
    ...
    def save_report_to_disk(self, generated_report: GeneratedReport, 
                            fmt: ReportFormat, custom_dir: Optional[str] = None) -> str:
